...
initial begin
    ...
    m0_rdall_m1_wrall();
    repeat(2) @(posedge clk);
    // ---- TC9 Write all positions of address with M1 and M0 at the same time with differrent address ----
    adif_m0_wrall_m1_wrall();
    repeat(2) @(posedge clk);
    // ---- TC10 Read all positions of address with M1 and M0 at the same time with differrent address ----
    adif_m0_rdall_m1_rdall();
    repeat(2) @(posedge clk);
    // ---- TC11 Read all positions of address with M1 and write M0 at the same time with differrent address ----
    adif_m0_rdall_m1_wrall();
    repeat(2) @(posedge clk);
    // ---- TC12 Write all positions of address with M1 and read M0 at the same time with differrent address ----
